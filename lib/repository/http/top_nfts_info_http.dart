...
class TopNftsInfoHttp implements InterfaceTopNftsInfoRepository {
  ...
  Future<List<OwnedNftsModel>> findAllNfts() async {
    ...
    if (response.statusCode == 200) {
      List<OwnedNftsModel> ownedNfts = List<OwnedNftsModel>.from(
        jsonDecode(response.body)['ownedNfts'].map(
          (element) => OwnedNftsModel.fromMap(element),
        ),
      );
      return ownedNfts
          .where(
            (element) =>
                !element.metadata.image.contains('data:image/svg+xml;base64')).toList();
    } else {
